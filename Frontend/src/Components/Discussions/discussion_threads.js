...
const DiscussionThreads = ({ thread, currentUser }) => {
  ...
  const fetchProfile = async () => {
    try {
      const response = await fetch(
        `${config.endpoints.users}/${thread.posted_by}`,
        { credentials: "include" },
      );
  ...
  useEffect(() => {
    setCommentsData(thread.comments || []);
  }, [thread.comments]);
  ...
  const handleCommentSubmit = async (event) => {
    ...
    if (!currentUser) {
      toast.error("Log in first to Comment");
      return;
    }
    ...
    try {
      const response = await fetch(
        `${config.endpoints.discussions}/comment/${thread._id}`,
        {
          method: "POST",
  ...
  const handleContributionUpdate = async () => {
    ...
    try {
      const response = await fetch(
        `${config.endpoints.hof}/thread_comment/${currentUser._id}`,
  ...
  const handleLike = async () => {
    if (!currentUser) {
      toast.error("Log in first to Like");
      return;
    }

    const response = await fetch(
      `${config.endpoints.discussions}/like/${thread._id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  const handleDislike = async () => {
    if (!currentUser) {
      toast.error("Log in first to Dislike");
      return;
    }

    const response = await fetch(
      `${config.endpoints.discussions}/dislike/${thread._id}`,
      {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ userId: currentUser._id }),
          ...
          <div className="comments-container">
            ...
            {commentsData && commentsData.length > 0 ? (
              commentsData.map((comment_item, index) => (
                <Comment
                  key={comment_item._id || comment_item.createdAt || index}
